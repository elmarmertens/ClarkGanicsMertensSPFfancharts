...
DATALABELS = {'RGDP10', 'CPI10', 'TBILL10'};
...
for d =  1 : length(DATALABELS)
    ...
    switch datalabel10
        ...
        case {'CPI0510'}
            ...
            Ngap        = 25;
        otherwise
            error('datalabel10 <<%s>> not supported', datalabel10)
    end

    %% load data
    matfilename = fullfile(datadir, sprintf('kensington%sdata', upper(datalabel10)));
    ...
    if strcmpi(datalabel10, 'CPI0510')
        Nbar = Nbar - 2;
    else
        Nbar = Nbar - 1;
    end
    ...
    matfilename = fullfile(datadir,sprintf('kensington%sdataHISTOGRAMS.mat', upper(datalabel)));
    ...
    %% prepare wrapper
    if strcmpi(datalabel10, 'CPI0510')
        modellabel = strcat(datalabel, 'STATEtrendgaplongSV2');
    else
        modellabel = strcat(datalabel, 'STATEtrendgaplongSV');
    end
    ...
    hvardraws       = NaN(MCMCdraws,T);
    hrhodraws       = NaN(MCMCdraws,T);
    ...
    fcstYmvlogscoreDraws = NaN(T,NfcstDraws); % one-step ahead only
    fcstYmvlogscore      = NaN(T,1);          % one-step ahead only


    [YFINALdraws, SVFINALdraws, ...
        ETAFINALdraws, ETASVFINALdraws] = deal(cell(T,1)); % trick to collect only thisT==T output inside parfor
    ...
    parfor thisT = Tstart : T % @parfor
        ...
        while ~OK
            try
                [mcmc_Ydensitydraws, mcmc_Y, mcmc_ETA, mcmc_ETASV, ...
                    mcmc_SV, mcmc_hVAR, mcmc_RHO, mcmc_sqrtSIGMA, mcmc_mustarVAR, ...
        ...
        hvardraws(:,thisT)               = mcmc_hVAR;
        hrhodraws(:,thisT)               = mcmc_RHO;
        mustarvoldraws(:,thisT)          = sqrt(mcmc_mustarVAR);
        ...
        fcstYhatRB(thisT,:)       = mean(mcmc_Y(2:end,end,:),3);   % reflecting MDS assumption
        ...
        if  thisT == T
            ...
            ETASVFINALdraws{thisT}      = mcmc_ETASV;
            SVFINALdraws{thisT}         = mcmc_SV;
        else
            YFINALdraws{thisT}      = [];
            ETAFINALdraws{thisT}    = [];
            ETASVFINALdraws{thisT}  = [];
            SVFINALdraws{thisT}     = [];
        end
    ...
    ETASVFINALdraws = ETASVFINALdraws{T};
    ...
    fcstYCURRENTpits(isnan(Yfuture)) = NaN;
    fcstYCURRENTpits(1:Tstart-1,:)   = NaN;

    fcstYAVGpits                 = mean(YAVGdensityDraws < YAVGfuture, 3);
    fcstYAVGpits(isnan(Yfuture)) = NaN;
    fcstYAVGpits(1:Tstart-1,:)   = NaN;

    %% plot comparison of log scores (ksdensity vs RB)
    T2015 = find(dates == datenum(2015,10,1));
    for jj= 1 : Nhorizons
        lhs = fcstYlogscoreRB(:,jj);
        rhs = fcstYlogscore(:,jj);
        thisfig = figure;
        subplot(2,1,1)
        plot(lhs,rhs, 'x')
        xlabel('RB')
        ...
        title('all')
        subplot(2,1,2)
        plot(fcstYlogscoreRB(1:T2015,jj), fcstYlogscore(1:T2015,jj), 'x')
        xlabel('RB')
        ylabel('Kernel  density')
        title('until 2015')
        sgtitle(sprintf('%s: h = %d', datalabel, jj))
        wrapthisfigure(thisfig, sprintf('logscoreComparison-%s-h%d', datalabel, jj), wrap)
    end
