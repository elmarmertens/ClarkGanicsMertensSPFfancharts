...
VARpriorshrinkage = 10; % will be divided by 1000
...
for d =  1 : length(DATALABELS)
    ...
    %% prepare wrapper
    modellabel = strcat(datalabel, sprintf('VARprior%dSTATEtrendgapSV', VARpriorshrinkage));
    ...
    %% prepare prior for G
    Ngap   = Ny - 1;
    G0var  = NaN(Ngap*Ngap,1); % diagonal elements
    Gtheta    = [0 0];
    ...
    Gdraws          = NaN(Ngap,Ngap,MCMCdraws,T);
    ...
    parfor thisT = Tstart : T % @parfor
        ...
        [mcmc_Ydensitydraws, mcmc_Y, mcmc_YhatRB, mcmc_ETA, ...
            mcmc_G, ... % Gdraws
            mcmc_SV, mcmc_hVAR, mcmc_RHO, ~, ~, ...
            YstateTp1mean, YstateTp1var, yTpHmean, yTpHvar ] = ...
            mcmcsamplerVARSTATEtrendgapSV(Zdata, Znanny, Cz, ...
            G0var, ...
    ...
    Gprior = reshape(Gprior, Ngap, Ngap, MCMCdraws);
