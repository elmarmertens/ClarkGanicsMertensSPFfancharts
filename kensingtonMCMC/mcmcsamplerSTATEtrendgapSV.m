function [YdensityDraws, Ydraws, ETAdraws, ETASVdraws, ...
    ...
    ] = mcmcsamplerSTATEtrendgapSV(Z, Znanny, Cz, ...
...
for mm = -MCMCdraws : MCMCdraws
    ...
    etaScaled = (etaGAP ./ SVscale)'; % could also use SVscale with automatic matrix expansion
...
for mm = 1 : MCMCdraws
    for jj = 1 : T
        thisB                  = zeros(Ny,Ny);
        thisB(:,Ny)            = mustarVAR(mm); % recall: mustarVAR stored VOL (will be converted to VAR only later)
        thisB(1:Ny-1,1:Ny-1)   = SVSCALEdraws(jj,mm) * sqrtSIGMA(:,:,mm);
