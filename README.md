...
To generate the tables and figures presented in the EntropicTilting paper, the results generated by the scripts in `kensingtonET` and stored in `.mat` files are also needed, in addition to the MCMC results. 
...
- `tabulateRelativeEvalstatsETSPFCDFforPaper.m`: forecast accuracy comparison table (SV and CONST models, tilting to SPF histograms vs. tilting to mean, variance and skewness of fitted generalized beta distribution, Table 4).
