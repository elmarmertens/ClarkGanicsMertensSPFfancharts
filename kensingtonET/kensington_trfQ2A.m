function [YBARdensityDraws,YCURRENTdensityDraws] = kensington_trfQ2A(doNIPA,thisT,datesQ,RT_obsdate_idx,YdensityDraws,RT_vec,Ndraws,fc_maxhz)
...
if doNIPA==1
    ...
    last_year_data=RT_vec(last_Q4-3:last_Q4,1); % only need last year's data when working with growth rates
...
this_Q1=last_Q4+1;
this_year_data=repmat(RT_vec(this_Q1:RT_obsdate_idx,1),1,Ndraws);
% extract current year's forecasts
this_year_fc=reshape(squeeze(YdensityDraws(thisT,1:5-Q_act,1:Ndraws)),[],Ndraws); % reshaping is only effective if we extract a COLUMN vector
...
if doNIPA==1
    ...
    B=ones(1,Ndraws);
    for k=2:4
        B=B+exp(sum(this_year_data_fc(2:k,1:Ndraws),1)/400);
    end

    A=exp(repmat(sum(last_year_data(2:4,1))/400,1,Ndraws)+this_year_data_fc(1,1:Ndraws)/400);

    YCURRENTdensityDraws=(A.*B./C - 1) * 100;

else

    YCURRENTdensityDraws=sum(this_year_data_fc,1)/4;
end
...
YBARdensityDraws=NaN(fc_maxhz,Ndraws);
...
for hz=1:fc_maxhz
    ...
    future_fc=squeeze(YdensityDraws(thisT,future_start:future_end,1:Ndraws));
    ...
    if doNIPA==1
        ...
        for k=2:4
            B=B+exp(sum(future_fc(2:k,1:Ndraws),1)/400);
        end

        A=exp((sum(A_term1(2:4,1:Ndraws),1)+future_fc(1,1:Ndraws))/400);
        ...
        YBARdensityDraws(hz,1:Ndraws)=sum(future_fc,1)/4;
