%% MDS-TRENDCYCLE-SV model with horseshoe shocks to trend and multivariate SV-t-scale, and 2block SV and noiseHS
% uses MDS assumption
...
doY1Q4             = true; % do include Y1Q4 (default)
...
for d =  1 : length(DATALABELS)
    ...
    if doTBILLcensored & strcmpi(datalabel, 'TBILL')
        modellabel = sprintf('%scensored-MDStrendHScycleSVt2blockNoiseHS-Ngap%s-samStart%s', datalabel, NGAP, datestr(dates(samStart), 'yyyyQQ'));
        doCENSOR   = true;
    else
        modellabel = sprintf('%s-MDStrendHScycleSVt2blockNoiseHS-Ngap%s-samStart%s', datalabel, NGAP, datestr(dates(samStart), 'yyyyQQ'));
        doCENSOR   = false;
    end
    ...
    if ~doSPFquarterlyOnly
        ...
        warning('doSPFquarterlyOnly=true not needed for noise model')
    ...
    parfor thisT = Tstart : T % @parfor
        ...
        if thisNstates > Nhorizons
            ...
            yhatRB                   = NaN(Nhorizons,1);              % parfor helper
            yhatRB(1:thisNstates-1)  = mean(mcmc_Y(2:end,end,:),3);
