%% VAR-TRENDCYCLE-SV model with horseshoe shocks to trend and multivariate SV-t-scale, and 2block SV and noiseHS
...
for d =  1 : length(DATALABELS)
    ...
    if doTBILLcensored & strcmpi(datalabel, 'TBILL')
        modellabel = sprintf('%scensored-VAR0trendHScycleSVt2blockNoiseHS-Ngap%s-samStart%s', datalabel, NGAP, datestr(dates(samStart), 'yyyyQQ'));
        doCENSOR  = true;
    else
        modellabel = sprintf('%s-VAR0trendHScycleSVt2blockNoiseHS-Ngap%s-samStart%s', datalabel, NGAP, datestr(dates(samStart), 'yyyyQQ'));
        doCENSOR   = false;
    end
            ...
            try
                [mcmc_Ydensitydraws, mcmc_Y, mcmc_YhatRB, mcmc_ETA, ~, ~, ...
                    ...
                    Ztp1mean, Ztp1meanerror, Zmvlogscore] = ...
	                     mcmcsamplerVAR0trendHScycleSVt2blockNoiseHS(Zdata, Znanny, Cz, thisNgap, ...
                    thisG0var, ...
                    ndxZbar, dates, ...
    ...
    parfor tt = Tstart : T
        ...
        % prior
        Gprior = chol(diag(G0var{tt}), 'lower') * randn(Ng, MCMCdraws);
