function [YdensityDraws, Ydraws, YhatRB, ETAdraws, RESIDSVdraws,ZRESIDdraws, ...
    ...
    ] = mcmcsamplerVAR0trendHScycleSVt2blockNoiseHS(Z, Znanny, Cz, Ngap, ...
    G0var, ...
    ndxZbar, dates, ...
...
if nargin < 12 || isempty(showProgress)
    showProgress = false;
end
if nargin < 13 || isempty(thinMax)
    thinMax = 1;
end
...
ndxVecG1  = transpose(1 : svN11 * Ngap);
ndxVecG2  = transpose(svN11 * Ngap + (1 : svN22 * Ngap));
...
if isempty(G0var)
    ...
    if ~isvector(G0var)
        if ~isdiag(G0var)
            error('G0var should be diagonal (only spherical priors supported)')
        end
        G0var = diag(G0var);
    end
end

% invG0var   = spdiags(1 ./ G0var(:), 0, Ng, Ng);
invG0var   = 1 ./ G0var(:);
gP11       = spdiags(invG0var(ndxVecG1), 0, svN11 * Ngap, svN11 * Ngap);
gP22       = spdiags(invG0var(ndxVecG2), 0, svN22 * Ngap, svN22 * Ngap);
...
NgibbsVAR2block         = 10; % Gibbs-within-Gibbs steps for 2block-triangularVAR
mm                      = 0;
thisMCMCdraw            = 0;
thinStep                = 0;
maxRepeatedRejections   = 10;
...
while thisMCMCdraw < MCMCdraws
    ...
    if OK
        ...
        statevol         = cholP22bar \ QQ2(:,ndxStateT); % note: statevol is not square!
    ...
    if OK
        ...
        invsigma22     = transpose(invsqrtsigma22) * invsqrtsigma22;

        % two-block triangular VAR
        [GPREV, residGAP] = bayesRegressionSV2block(GPREV,  etaGAP, etaGAPlag, NgibbsVAR2block, invSVt, invsigma11, invsigma22, slopes12PREV, gP11, gP22, ndxSV11, ndxSV22, Igap, rndStream);
    ...
    if OK
        ...
        rhs                            = residGAP(:,ndxSV22) .* invSVt(:,1); 
        % invsigma11                     = transpose(invsqrtsigma11) * invsqrtsigma11; % already computed above
