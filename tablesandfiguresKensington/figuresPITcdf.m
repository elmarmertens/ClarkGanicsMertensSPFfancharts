...
samStartLabel      = '1968Q4';
MDStype            = {'MDS','VAR0'};
SHOCKtype          = {'trendHScycleSVtnoise', 'trendHScycleSVtnoiseHS', 'trendcycleSVtnoiseHS', 'trendHScycleSVt'};
SPFquarterly       = {'','-y1q4','-SPFquarterlyOnly'};
prettySPFquarterly = {'excl Q4 of y1','incl Q4 of y1', 'fixed h only'};

MDStype            = {'MDS','VAR0'};
SHOCKtype          = {'trendHScycleSVtnoiseHS', 'trendHScycleSVt2blocknoiseHS'};
SAMSTART = {'1968Q4'};
    ...
    for mm = 1 : length(MDStype)
        ...
        for ss = 1 : length(SHOCKtype)
            for qq = 1 : length(SPFquarterly)
                ...
                models(nn,mm,ss,qq).pretty    = sprintf('%s', MDSpretty{mm});
...
for mm = 1 : 2 : length(MDStype)
    for qq = 1 : length(SPFquarterly)
        ...
        if isempty(SPFquarterly{qq})
            grouplabels = cat(1, grouplabels, '-Xy1q4');
        else
        ...
        for g =  1 : numel(groups)
            ...
            if ~all(modelsNaN(thesemodels))
                ...
                for h = 1 : Nhorizons
                    ...
                    wrapthisfigure(thisfig, sprintf('PITs-%s-h%d-%s%s-WITHLEGEND', datalabel, h-1, sam(s).label, grouplabels{g}), wrap);
                    delete(hl)
                    wrapthisfigure(thisfig, sprintf('PITs-%s-h%d-%s%s', datalabel, h-1, sam(s).label, grouplabels{g}), wrap);                    
                end
            end
