...
% also collecting calendar-year growth rates (RGDP, PGDP) and annual averages (UNRATE, CPI, TBILL)
clear; close all; clc;
...
%% TBILL (straight from FRED)
% no real-time files needed, constructed here just for sake of symmetry in the code

FRED           = fredreadcsv('TB3MS',[], 'm', 'q', 'avg');
obsdates_TBILL = datetime(FRED.DATES, 'ConvertFrom','datenum');
vindates_TBILL = obsdates_TBILL(end);

TBILL.data     = FRED.VALUES;
TBILL.obsdates = obsdates_TBILL;
TBILL.vindates = vindates_TBILL;

% calculate calendar-year annual AVERAGE levels
TBILL_timetable = array2timetable(TBILL.data(:,end),"RowTimes",TBILL.obsdates);
TBILL_calY_avg_timetable = retime(TBILL_timetable,'yearly',@(z)mean(z,'includenan'));

TBILL_calY_avg = TBILL_calY_avg_timetable.Var1;
TBILL_calY_avg_dates = year(TBILL_calY_avg_timetable.Time);

TBILL.calY_avg = TBILL_calY_avg;
TBILL.calY_dates = TBILL_calY_avg_dates;

%% save RT data --- joint file

save('kensingtonRTDSM.mat','RGDP','PGDP','UNRATE','CPI','TBILL', '-v7.3');
