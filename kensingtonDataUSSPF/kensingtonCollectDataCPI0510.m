...
%% process SPFimport10 and 5
SPF10import = importdata('Mean_CPI10_Level.xlsx'); % this is the original xls file converted into csv
...
SPF05import = importdata('Mean_CPI5YR_Level.xlsx'); % this is the original xls file converted into csv
SPF05import.data(SPF10import.data == -999) = NaN;

% check dates
Y = SPF05import.data(:,1);
if ~isequal(Y, year(dates))
    error('inconsistent date vector in the SPF file (years)');
end
Q = SPF05import.data(:,2);
if ~isequal(Q, quarter(dates))
    error('inconsistent date vector in the SPF file (quarters)');
end
...
Ybarforecast05     = SPF05import.data(:,3);
...
Nz3        = 2;
Zdata3     = [Ybarforecast05 Ybarforecast10];
Z3label    = {'hbar-20'; 'hbar-40'};
C3         = zeros(Nz3,Ny,T);
for t = 1 : T

    if ~all(isnan(Zdata3(t,:)))
        
        ndxMA40          = (0:39 - (Q(t) - 1)) + 2;  % add 2 for nowcast and lagged realized
        ndxMA20          = (0:19 - (Q(t) - 1)) + 2;  % add 2 for nowcast and lagged realized

        ndxMA40 = ndxMA40(ndxMA40 > 0);
        ndxMA20 = ndxMA20(ndxMA20 > 0);

        switch Q(t)
            case {1, 2}
                % ndx jump off counts quarters left in current
                C3(1,ndxMA20,t)  = 0.05; % = 1 / 20;
                C3(2,ndxMA40,t)  = 0.025; % = 1 / 40;
            case 3
                inflationQ1      = Yrealized(t-3); % recall that Yrealized containes lagged inflation
                Zdata3(t,1)      = (20 * Zdata3(t,1) - inflationQ1) / 19;
                Zdata3(t,2)      = (40 * Zdata3(t,1) - inflationQ1) / 39;
                C3(1,ndxMA20,t)  = 1 / 19;
                C3(2,ndxMA40,t)  = 1 / 39;
            case 4
                inflationQ1      = Yrealized(t-4); % recall that Yrealized containes lagged inflation
                inflationQ2      = Yrealized(t-3); % recall that Yrealized containes lagged inflation
                Zdata3(t,1)      = (20 * Zdata3(t,1) - inflationQ1 - inflationQ2) / 18;
                Zdata3(t,2)      = (40 * Zdata3(t,1) - inflationQ1 - inflationQ2) / 38;
                C3(1,ndxMA20,t)  = 1 / 18;
                C3(2,ndxMA40,t)  = 1 / 38;
        end
    end
...
for t = 1 : T
    if rank(Cz(~Znanny(t,:),:,t)) == 0
        warning('zero Rank C at t=%d', t)
    end
    if abs(det(Cz(~Znanny(t,:),:,t)*Cz(~Znanny(t,:),:,t)')) < 1e-6
        warning('singular C at t=%d', t)
    end
end
...
YforecastLabel(maxHorizon+2:end-2) = YBARlabel;
YforecastLabel{end-1} = '5-year';
