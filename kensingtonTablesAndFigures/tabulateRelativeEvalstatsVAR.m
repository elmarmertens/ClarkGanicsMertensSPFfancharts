...
models(m).pretty = 'MDS (SV)';
models(m).Ndraws = 3e3;

% m = m + 1;
% models(m).type   = 'VARprior5STATEtrendgapSV';
% models(m).pretty = 'Non-MDS (SV)';
% models(m).Ndraws = 3e3;

m = m + 1;
models(m).type   = 'VARprior10STATEtrendgapSV';
models(m).pretty = 'Non-MDS (tight; SV)';
models(m).Ndraws = 3e3;

m = m + 1;
models(m).type   = 'STATEconst';
models(m).pretty = 'MDS (CONST)';
models(m).Ndraws = 3e3;

m = m + 1;
models(m).type   = 'VARprior10STATEconst';
models(m).pretty = 'Non-MDS (CONST)';
models(m).Ndraws = 3e3;
...
sam(s).pretty = '1992-2022';

s = 4;
sam(s).start  = datenum(1992,1,1);
sam(s).stop   = datenum(2016,12,1);
sam(s).label  = 'preCOVIDsince1992';
sam(s).pretty = '1992-2016';
...
% PAIRS = cat(2, PAIRS, [3 5]);

%% latexwrapper
wrap   = [];
titlename = 'tabulateRelativeEvalStatsVAR';
if doPlotsCumLoss
...
for pp = 1 : length(PAIRS)
    ...
    for s = 1  : length(sam)
        ...
        for d = 1 : length(DATALABELS)
            ...
            Tstart        = max(mat0.Tstart, mat1.Tstart);
            ...
            %% MAE
            %             loss0 = abs(mat0.fcstYmederror);
            %             loss1 = abs(mat1.fcstYmederror);
            %             loss0(~sammy,:) = NaN;
            %             loss1(~sammy,:) = NaN;
            %
            %             for hh = 1 : theseNhorizons
            %                 if max(abs(loss0(:,hh) - loss1(:,hh))) > 1e-6 % for some horizons, means are (virtually) equal
            %                     [~, relMAEtstat(hh,d)] = dmtest(loss0(:,hh), loss1(:,hh), hh + 1);
            %                     relMAEtstat(hh,d) = 0;
            %                     %                     warning('virtually identical medians (h=%d, %s)', hh, datalabel)
            %                 end
            %             end
            %
            %             MAE0(1:theseNhorizons,d) = mean(loss0,1, 'omitnan');
            %             MAE1(1:theseNhorizons,d) = mean(loss1,1, 'omitnan');
...
fprintf(fid, '%s \n', comparisonNote);
fprintf(fid, 'Variable mnemonics: UNRATE denotes the unemployment rate, RGDP real growth, TBILL the interest rate on Treasury bills, CPI inflation, and PGDP changes in the GDP deflator. (All growth rates are expressed as annualized percentage points of quarterly rates of change.)\n');
