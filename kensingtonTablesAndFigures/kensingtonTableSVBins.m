...
datalabel  = 'RGDP'; % adjust table note in line 439!
fontsize   = 18;
...
models(m).pretty  = 'CONST w/ET (SV)';
models(m).Ndraws  = 3e3;
models(m).ETlabel = 'SVBins';
...
%% latexwrapper
wrap   = [];
titlename = strcat('kensingtonTable1SVBins-', datalabel);
...
switch ETlabel0
    ...
    case 'binsAndMeans'
        ...
        isET0 = true;
    case 'SVBins'
        modelpretty0 = sprintf('ET (SV probabilities) with %s', models(m0).pretty);
        isET0 = true;
    otherwise
...
switch ETlabel0
    ...
    case {'binsOnly', 'binsAndMeans','SVBins'}
        ...
        matfilename   = sprintf('slimCGMmcmc-%s-Ndraws%d', modellabel0, Ndraws0);
        matbase0      = matfile(fullfile(resultdir, matfilename));
    otherwise
        error('ETlabel <<%s>> not recognized', ETlabel0)
end
...
tabname    = sprintf('tableSVBins-%s.tex', datalabel);
tabcaption = tabname;
