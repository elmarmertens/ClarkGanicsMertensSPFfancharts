...
titlename = 'SPFdrps';
titlename = strcat(titlename, '-WithCURRENT');
...
%% allocate memory
maxNbar = 3; 
maxNbar = maxNbar + 1; % add 1 for current
...
for mm = 1 : length(models)
    ...
    for dd = 1 : length(DATALABELS)
        ...
        theseNbar   = theseNbar + 1; % add CURRENT
        ...
        for ss = 1 : length(samples)
            ...
            loss0 = [PROBdata.fcstSPFdrpsCURRENT PROBdata.fcstSPFdrps];
            loss1 = [mat0.fcstYCURRENTdrps mat0.fcstYBARdrps];
...
for dd = 1 : length(DATALABELS)
    ...
    for hh = 1 : maxNbar
        ...
        if hh <= 2 % skip first two samples (since 1992) for hh > 2
...
for dd = 1 : length(DATALABELS)
    ...
    for hh = 1 % : maxNbar
        ...
        orient landscape
        wrapthisfigure(thisfig, sprintf('DRPS-%s-y%d', datalabel, hh-1), wrap, [], [], [], [], true)
        legend(hanni, modellist, 'location', 'northwest', 'fontsize', 22)
        wrapthisfigure(thisfig, sprintf('DRPS-%s-y%d-WITHLEGEND', datalabel, hh-1), wrap)
    end
    % growing averages
    for hh = 1 % : maxNbar
        thisfig = figure;
        ...
        xtickdates(thesedates)
        orient landscape
        wrapthisfigure(thisfig, sprintf('avgDRPS-%s-y%d', datalabel, hh-1), wrap, [], [], [], [], true)
        legend(hanni, modellist, 'location', 'best', 'fontsize', 22)
        wrapthisfigure(thisfig, sprintf('avgDRPS-%s-y%d-WITHLEGEND', datalabel, hh-1), wrap)
    end
    ...
    %         plotOrigin
    %         orient landscape
    %         wrapthisfigure(thisfig, sprintf('diffavgDRPS-%s-y%d', datalabel, hh-1), wrap, [], [], [], [], true)
    %         legend(hanni(2:end), modellist(2:end), 'location', 'best', 'fontsize', 22)
    %         wrapthisfigure(thisfig, sprintf('diffavgDRPS-%s-y%d-WITHLEGEND', datalabel, hh-1), wrap)
    %     end
end
...
for dd = 1 : length(DATALABELS)
    ...
    for hh = 1 : maxNbar
        ...
        csvname = sprintf('DRPS-%s-y%d', datalabel, hh-1);
        writedatatable(wrap, csvname, dates, outx, modellist, 'yyyyQQ');
        % AVGLOSS
        outx    = squeeze(AVGLOSS(:,hh,ss,dd,:));
        csvname = sprintf('avgDRPS-%s-y%d', datalabel, hh-1);
        writedatatable(wrap, csvname, dates, outx, modellist, 'yyyyQQ');
    end
end
