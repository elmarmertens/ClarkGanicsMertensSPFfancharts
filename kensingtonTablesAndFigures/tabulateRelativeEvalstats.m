...
m = m + 1;
models(m).type   = 'STATEtrendgapSV';
models(m).pretty = 'MDS (SV)'; % same as m=1 just with different pretty
models(m).Ndraws = 3e3;
...
m = m + 1;
models(m).type   = 'STATEtrendgapCONST';
models(m).pretty = 'TRENDGAP-CONST';
models(m).Ndraws = 3e3;

m = m + 1;
models(m).type   = 'VARprior10STATEtrendgapCONST';
models(m).pretty = 'VAR-TRENDGAP-CONST';
models(m).Ndraws = 3e3;
...
fprintf(fid, '%s \n', comparisonNote);
fprintf(fid, 'Variable mnemonics: RGDP denotes real growth, UNRATE the unemployment rate, PGDP changes in the GDP deflator, CPI inflation, and TBILL the interest rate on Treasury bills. (All growth rates are expressed as annualized percentage points of quarterly rates of change.)\n');
