...
for d = 1 : length(DATALABELS)
    ...
    for s = 1  : Nsam
        ...
        nanny         = isnan(loss0)  | isnan(loss1)  | isnan(loss2)  | (~sammy);
...
%% tabulate
tabname    = sprintf('table3-%s-%s.tex', DATALABELS{1}, DATALABELS{2});
tabcaption = tabname;
...
% data header
for d = 1 : Nvar
    fprintf(fid, '& \\multicolumn{%d}{c}{%s} ', Nsam * Nmodels, DATALABELS{d});
end
fprintf(fid, '\\\\\n');
that = 1;
fprintf(fid, '\\cmidrule(lr){%d-%d}', that + 1, that + Nsam * Nmodels);
that = 1 + + Nsam * Nmodels;
fprintf(fid, '\\cmidrule(lr){%d-%d}', that + 1, that + Nsam * Nmodels);

% model header
fprintf(fid, '& \\multicolumn{2}{c}{%s} ', modelpretty1);
fprintf(fid, '& \\multicolumn{2}{c}{%s} ', modelpretty2);
fprintf(fid, '& \\multicolumn{2}{c}{%s} ', modelpretty1);
fprintf(fid, '& \\multicolumn{2}{c}{%s} ', modelpretty2);
fprintf(fid, '\\\\\n');
for nn = 1 : Nmodels * 2
...
fprintf(fid, '\\begin{small}\n');
fprintf(fid, 'Note: \n');
fprintf(fid, 'RMSE and CRPS of each model relative to %s (in denominator).\n', modelpretty0);
